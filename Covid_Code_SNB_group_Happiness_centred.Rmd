---
title: "Covid_Code_SNB_group_Hapcentred"
...
14. combine happiness with Stringency, centre happiness
...
17. visualizing Stringency vs Happiness per year 2020-2022
...
18. visualizing Happiness per country and per region over time 2019-2022 transpose first
...
19. visualizing Stringency per country and region over time 2020-2022 transpose first
...
hapsting_by_deaths <- total_data %>%
  ...
  theme_minimal() +
  scale_colour_gradientn(colors=rainbow(7)) +
  labs(title="Influence of Stringency on Happiness", subtitle="Happiness change from pre-pandemic levels") +
  theme(legend.position = "bottom", legend.text = element_text(size = 5)) +
  facet_wrap(~year, scale="fixed") + stat_smooth(method = 'lm')
hapsting_by_deaths
```
...
```{r echo=FALSE, fig.height=30, fig.width=45, warning=FALSE, include=FALSE}
library(ggrepel)
#total_data %>%
...
```{r fig.height=30, fig.width=40, include=FALSE, warning=FALSE, echo=FALSE}
