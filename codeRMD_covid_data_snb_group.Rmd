...
ggplot(aes(x = Stringency, y = Happiness, label = country_code, colour = Region)) +
  ...
  geom_text_repel(aes(label = country_code), size = 3, 
                   segment.color = 'grey') +
  geom_point(aes(size = deathsdiff)) +
  facet_wrap(~year, scale = "fixed") + theme_gray(base_size = 30) + theme(panel.background = element_rect(fill = "white"),plot.background = element_rect(fill = "grey95"), panel.grid.major = element_line(color = 'grey90', linetype = 'dotted'))
...
total_data_wo_na_2019 %>%
  ...
  geom_point(aes(size = deathsdiff))+
  theme_bw() +
  facet_wrap(~ Region * year) + stat_smooth(method = "lm", se=FALSE) + theme_gray(base_size = 30) + theme(panel.background = element_rect(fill = "white"),plot.background = element_rect(fill = "grey95"), panel.grid.major = element_line(color = 'grey90', linetype = 'dotted'))
...
```{r fig.height=30, fig.width=40, warning=FALSE}
...
total_new %>%
  ...
  geom_point(aes(size = deathsdiff))+
  theme_minimal()+
  facet_wrap(~ Continent * year, ncol = 3) + stat_smooth(method = "lm", se=FALSE) + theme_gray(base_size = 30) + theme(panel.background = element_rect(fill = "white"),plot.background = element_rect(fill = "grey95"), panel.grid.major = element_line(color = 'grey90', linetype = 'dotted'))
